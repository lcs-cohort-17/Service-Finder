...
// Note: the error responses here no longer format their own JSON — they
// throw an AppError and let the centralized errorHandler (registered
// last in index.js) turn it into the response. The { status, error:
// { code, message } } shape stays identical to before; it's just built
// in one place now instead of once per controller. Express 5 forwards
// a thrown/rejected error from an async handler to that middleware
// automatically, so no try/catch-and-call-next boilerplate is needed
// for the validation/not-found/upstream branches below — only the
// fetch itself is wrapped, since that's the one call that can reject
// for reasons (network failure, abort) we need to distinguish.
async function calculateRoute(req, res) {
  ...
  if (validationError) {
    ...
    throw new AppError(validationError, 400, "INVALID_REQUEST");
  }
  ...
  try {
    ...
    // AppErrors thrown above (upstream_error / not_found) just need to
    // pass through to Express's error forwarding, not be re-wrapped.
