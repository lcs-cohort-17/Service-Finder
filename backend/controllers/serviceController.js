import {  getApprovedServicesWithBounds, getDeclinedServicesDb, getPendingServicesDb, getUserApprovedServicesDb, getUserDeclinedServicesDb, getUserPendingServicesDb, saveUserServiceDb, seedFromOverpassDb, subscribeToApprovedServices, updateServiceStatusDb } from "../models/serviceModel.js";
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: 'Missing required query parameters: minLat, maxLat, minLng, maxLng.',
      });
    }

    // Call the new model function
    const services = await getApprovedServicesWithBounds({
    ...
    // Handle quota exceeded (Firestore errors)
    const quotaExceeded =
      error.code === 8 ||
...
export const streamApprovedServices = (req, res) => {
  ...
  const unsubscribe = subscribeToApprovedServices(
    {
      minLat: Number(minLat),
      maxLat: Number(maxLat),
      minLng: Number(minLng),
...
export const getPendingCurrentUserSuggestedServices = async (req, res, next) => {
  try {
    const email = req.user.email; // From verifyToken
    const limitAmount = parseInt(req.query.limit, 10) || 20;

    const services = await getUserPendingServicesDb(email, limitAmount);
...
export const getApprovedCurrentUserSuggestedServices = async (req, res, next) => {
  try {
    const email = req.user.email;
    const limitAmount = parseInt(req.query.limit, 10) || 20;

    const services = await getUserApprovedServicesDb(email, limitAmount);
...
export const getDeclinedCurrentUserSuggestedServices = async (req, res, next) => {
  try {
    const email = req.user.email;
    const limitAmount = parseInt(req.query.limit, 10) || 20;

    const services = await getUserDeclinedServicesDb(email, limitAmount);
