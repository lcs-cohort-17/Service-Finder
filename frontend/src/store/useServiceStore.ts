...
  error: string | null;
  ...
  fetchAllRejectedServices: () => Promise<void>;
...
      set({
        approvedServices: normalizedServices,
...
fetchAllPendingServices: async () => {
    try {
      ...
      set({
        pendingServices: normalizedServices,
        loading: false,
      });
    } catch (err: any) {
...
fetchAllRejectedServices: async () => {
    try {
      set({ loading: true, error: null });
 
      const token = useAuthStore.getState().token;
 
      const response = await fetch(`${API_BASE}/declined`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      set({
        rejectedServices: normalizedServices,
        loading: false,
      });
    } catch (err: any) {
  ...
  moderateSuggestedServices: async (id, status) => {
    try {
      ...
      set((state) => ({
        pendingServices: state.pendingServices.filter((service) => service.id !== id),
      }));
    } catch (err: any) {
